...
<div class="table-card">
    ...
    <div style="overflow-x:auto;">
        <table class="data-table theme-development">
            <thead>
                <tr>
                    <th>No.</th>
                    <th>LGU</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="action-cell" style="text-align:center;">
                        <button class="btn-edit" onclick="openEditModal({{ $r->id }}, '{{ addslashes($r->lgu_name) }}', {{ $r->children_in_school_male ?? 'null' }}, {{ $r->children_in_school_female ?? 'null' }}, {{ $r->children_in_school_total ?? 'null' }}, '{{ addslashes($r->remarks ?? '') }}')">
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            Edit
                        </button>
                        <form method="POST" action="{{ route('add.development.destroy', $r->id) }}" style="display:inline;" onsubmit="return confirm('Delete development record for {{ addslashes($r->lgu_name) }}?')">
                            @csrf @method('DELETE')
