...
<div style="display:flex; gap:12px; flex-wrap:wrap; margin-bottom:20px; align-items:center;">
    ...
    <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
        <a href="{{ route('exports.download', ['dataset' => 'ip', 'format' => 'pdf']) }}" target="_blank" class="btn btn-outline btn-sm">Print</a>
        <a href="{{ route('exports.download', ['dataset' => 'ip', 'format' => 'excel']) }}" class="btn btn-outline btn-sm">Excel</a>
    </div>
</div>
...
<div class="table-card">
    ...
    <div style="overflow-x:auto;">
        <table class="data-table theme-ip">
            <thead>
                <tr>
                    <th>No.</th>
                    <th>LGU</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="num">{!! $r->male !== null ? number_format($r->male) : '<span class="pill pill-amber">N/A</span>' !!}</td>
                    <td class="num">{!! $r->female !== null ? number_format($r->female) : '<span class="pill pill-amber">N/A</span>' !!}</td>
                    <td class="num">
                        @if($r->total !== null)
                            @php $t = $r->total; $cls = $t >= 200 ? 'pill-red' : ($t >= 50 ? 'pill-amber' : 'pill-green'); @endphp
                            <span class="pill {{ $cls }}"><strong>{{ number_format($t) }}</strong></span>
                        @else
                            <span class="pill pill-amber">N/A</span>
                        @endif
                    </td>
                    <td class="action-cell" style="text-align:center;">
                        ...
                        <form method="POST" action="{{ route('add.ip.destroy', $r->id) }}" style="display:inline;" onsubmit="return confirm('Delete IP Children record for {{ addslashes($r->lgu_name) }}?')">
                            @csrf @method('DELETE')
...
function openEditModal(id, lgu, male, female, total) {
    document.getElementById('editForm').action = '/add/ip/' + id;
